...
function BinarySearchTree() {
  ...
  var insert_data = function insert_data(node, data) {
    if (root == null) {
  ...
  var search_data = function search_data(node, data) {
    if (node == null) {
      return null;
    }
  ...
  var link_parent = function link_parent(parent, node, next_node) {
    // 连接父节点和子节点
    if (parent == null) {
      root = next_node;
      root.parent = null;
  ...
  var remove_data = function remove_data(node, data) {
    if (node == null) {
      return false;
    }
    ...
    if (data < node.data) {
      ...
      if (node.leftChild && node.rightChild) {
        ...
        node.data = tmp.data; // 去右子树里删除中序下的第一个节点
  ...
  var tree_height = function tree_height(node) {
  ...
  var get_line = function get_line(offset, data) {
    var line = "";
  ...
  var set_line = function set_line(lines, index, line) {
    var old_line = lines[index];
    var sub_line = line.substring(old_line.length);
    lines[index] = old_line + sub_line;
  };

  var super_print = function super_print(node, height, curr_height, offset, direction, lines) {
    if (!node) {
      return;
    }
